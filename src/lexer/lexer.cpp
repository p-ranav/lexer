...
namespace lexer {
  ...
  void Lexer::ReadIdentifier() {
    ...
    if (tToken.mLiteral == "null") {
      ...
      tToken.mType = TokenType::TRUE_;
    }
    else if (tToken.mLiteral == "false") {
      tToken.mType = TokenType::FALSE_;
    }
  ...
  void Lexer::Tokenize() {
    for (mIndex = 0; mIndex < mSource.size();) {
      if (IsUTF8(mSource[mIndex])) {
        ...
        else if (tCharacter[0] == '\'') { // single-quoted string
          ReadSingleQuotedString();
        }
        else if (tCharacter[0] == '"') { // double-quoted string
